...
def animate_simulation(L_value, delta_t):
    """
    Analiza los datos de la simulación y crea una animación en Matplotlib
    usando un temporizador discreto (ej: cada delta_t unidades).
    """
    ...
    with open(params_file, "r") as f:
        ...
        L_CHANNEL, num_particles, radius, BOX1_WIDTH, BOX2_WIDTH, BOX1_HEIGHT = map(
            float, lines[0].split()
        )
        num_particles = int(num_particles)

    # --- Leer frames (formato: t en una línea, seguido de num_particles líneas con x,y,vx,vy) ---
    times = []
    frames = []
    ...
    print(f"Se leyeron {len(frames)} frames con {num_particles} partículas.")

    # --- Crear secuencia de tiempos uniformes ---
    
    t_min, t_max = times[0], times[-1]
    uniform_times = np.arange(t_min, t_max, delta_t)

    # Para cada tiempo uniforme, buscar el frame más cercano
    selected_indices = [
        np.argmin(np.abs(np.array(times) - ut)) for ut in uniform_times
    ]
    selected_frames = [frames[idx] for idx in selected_indices]

    print(
        f"Se seleccionaron {len(selected_frames)} frames uniformes de {t_min:.2f} a {t_max:.2f}"
    )
    ...
    # --- Crear partículas ---
    patches = [plt.Circle(xy, radius, fc="royalblue") for xy in selected_frames[0]]
    for patch in patches:
        ax.add_patch(patch)

    # --- Función de actualización ---
    def update(frame_index):
        positions = selected_frames[frame_index]
    ...
    ani = animation.FuncAnimation(
        fig, update, frames=len(selected_frames), blit=True, interval=30, repeat=False
    )
    ...
    print(f"Animación guardada exitosamente como 'simulation_L_{L_value}_.mp4'!")


if __name__ == "__main__":
    animate_simulation(0.05,delta_t = 0.05 )
